...
class BST {
    ...
    height(runner = this.root, branch = 0) {
        // If there is no node when the runner moves (or starts), add 0
        if (!runner) {
            return branch;
        }
        if (runner.left) {
            return this.height(runner.left, branch +1)
        }
        if (runner.right){
            return this.height(runner.right, branch + 1)
        }
    }

    

    heightbleh(runner = this.root){
        var branchHeight = 0
        if (!runner) {
            return 0;
        }
